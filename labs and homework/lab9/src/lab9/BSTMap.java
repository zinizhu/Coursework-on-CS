...
public class BSTMap<K extends Comparable<K>, V> implements lab9.Map61B<K, V> {
    ...
    private Set<K> keySets;/* Store all keys in the map.*/
    ...
    private Node remove(Node p, K key) {
        ...
        int cmp = key.compareTo(p.key);
        if (cmp < 0) {
            p.left = remove(p.right,key);
        }
        else if (cmp > 0) {
            p.right = remove(p.left,key);
        }
        else {
            if (p.right == null) {
                return p.left;
    ...
    public V remove(K key, V value) {
        V val = get(key);
        if (!val.equals(value)) {
            throw new NullPointerException("Value " + value + " is not linked to this key");
        }
        put(key,null);
        return value;
    }
